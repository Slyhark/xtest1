	...
	if( Event == CBNETProtocol :: EID_WHISPER || Event == CBNETProtocol :: EID_TALK || Event == CBNETProtocol :: EID_EMOTE || Event == CBNETProtocol :: CONSOLE_INPUT )
	{
		...
		else if( !Message.empty( ) && Message[0] == m_CommandTrigger[0] && Event != CBNETProtocol :: EID_EMOTE )
		{
			...
			********************************/
			...
			else if(( Command == "addadmin" || Command == "addhost" ) && Payload.empty( ) )	
				QueueChatCommand( m_CCBot->m_Language->ADDADMIN( User ), BNET ); 
			...
			else if(( Command == "version" || Command == "v" ) && Payload.empty( ) )					
			...
			else if(( Command == "ping" || Command == "p" ) && Access >= m_CCBot->m_DB->CommandAccess( "ping" ) )
			{
				if( Payload.empty( ) )
					Payload = User;			
